...
import io.reactivesw.order.infrastructure.enums.TransactionState;
import io.reactivesw.order.infrastructure.enums.TransactionType;
...
@ApiModel(description = "A representation of a financial transactions. " +
    "Transactions are either created by the solution implementation to trigger a new transaction " +
    "at the PSP or created by the PSP integration as the result of a notification by the PSP.")
@Getter
@Setter
public class TransactionModel {
  ...
  @ApiModelProperty(value = "The identifier that is used by the interface that managed the " +
      "transaction (usually the PSP). " +
      "If a matching interaction was logged in the interfaceInteractions array, the corresponding" +
      " interaction should be findable with this ID.",
      required = false)
